...
class CustomerModel {
    ...
    async getCustomerFilterOptions() {
        const segments = await pool.query("SELECT DISTINCT custsegment FROM leadtrackdetails WHERE custsegment IS NOT NULL AND custsegment != ''");
        const categories = await pool.query("SELECT DISTINCT compcat FROM leadtrackdetails WHERE compcat IS NOT NULL AND compcat != ''");
        const banks = await pool.query("SELECT DISTINCT bank FROM customers WHERE bank IS NOT NULL AND bank != ''");
        const loanTypes = await pool.query("SELECT DISTINCT product FROM customers WHERE product IS NOT NULL AND product != ''");
    ...
    async getTodayAppointment(empid) {
        const { rows } = await pool.query(
            "SELECT * FROM GetTodayAppoinment($1)",
            [empid]
        );
        return rows;
    }
